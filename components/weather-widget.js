/**
 * Weather Widget
 * Użycie: <script src="widget-pogoda.js?city=Warsaw,PL&threshold=20"></script>
 * 
 * Parametry URL:
 * - city: Miasto i kod kraju (np. "Warsaw,PL", "Berlin,DE", "Turkey" dla całego kraju)
 * - threshold: Próg temperatury w °C (domyślnie 20)
 * - lang: Język (domyślnie "pl")
 * - container: ID kontenera gdzie ma się pojawić widget (domyślnie tworzy własny)
 */

(function() {
    'use strict';
    
    // Pobierz parametry z URL skryptu
    function getScriptParams() {
        ...
        if (scriptSrc.includes('?')) {
            ...
            params.city = urlParams.get('city') || 'Warsaw,PL';
            params.threshold = parseInt(urlParams.get('threshold')) || 20;
            params.lang = urlParams.get('lang') || 'pl';
            params.container = urlParams.get('container') || null;
        }
        
        return params;
    }
    
    // Konfiguracja z parametrów URL
    const CONFIG = getScriptParams();
    CONFIG.apiKey = 'bd5e378503939ddaee76f12ad7a97608';
    CONFIG.updateInterval = 600000; // 10 minut
    
    // Unikalne ID dla tego widgetu
    const widgetId = 'weather-widget-' + Math.random().toString(36).substr(2, 9);
    ...
    const styles = `
        ...
        #${widgetId} .weather-city {
            font-size: 18px;
            font-weight: 500;
            color: #333;
        }

        #${widgetId} .weather-date {
            font-size: 13px;
            color: #999;
            margin-left: auto;
        }

        #${widgetId} .weather-main {
            display: flex;
            align-items: center;
            gap: 16px;
            margin-bottom: 16px;
        ...
        #${widgetId} .weather-temp {
            font-size: 32px;
            font-weight: 600;
            color: #333;
        }

        #${widgetId} .weather-icon-wrapper {
            display: flex;
            align-items: center;
        ...
        #${widgetId} .weather-night-temp {
            font-size: 20px;
            color: #666;
            white-space: nowrap;
        }

        #${widgetId} .weather-footer {
            display: flex;
            align-items: center;
            gap: 6px;
            padding-top: 12px;
        ...
        #${widgetId} .weather-footer-icon {
            width: 20px;
            height: 20px;
            color: #666;
        }

        #${widgetId} .weather-footer-text {
            font-size: 14px;
            color: #666;
        }

        #${widgetId} .weather-footer-temp {
            margin-left: 4px;
            color: #333;
        }
        ...
        #${widgetId} .icon-partly-cloudy {
            width: 48px;
            height: 30px;
            position: relative;
        }

        #${widgetId} .icon-partly-cloudy .sun-part {
            width: 28px;
            height: 28px;
            background: #FFD700;
            border-radius: 50%;
        ...
        #${widgetId} .icon-partly-cloudy .cloud-part {
            width: 35px;
            height: 20px;
            background: #90A4AE;
            border-radius: 10px;
        ...
        #${widgetId} .icon-partly-cloudy .cloud-part::before {
            content: '';
            position: absolute;
            width: 18px;
            height: 18px;
    ...
    const htmlTemplate = `
        <div class="weather-header">
            <span class="weather-city">Ładowanie...</span>
        ...
        <div class="weather-main">
            ...
            <span class="weather-night-temp">--°C</span>
        </div>
        ...
        <div class="weather-footer">
            ...
            <span class="weather-footer-temp">--°C</span>
        </div>
    `;
    ...
    function initWidget() {
        ...
        if (CONFIG.container) {
            ...
            if (container) {
                ...
                console.error(`Weather Widget: Container #${CONFIG.container} not found`);
                return;
            }
        } else {
            // Wstaw widget w miejscu skryptu
            const scripts = document.getElementsByTagName('script');
            const currentScript = scripts[scripts.length - 1];
            currentScript.parentNode.insertBefore(widget, currentScript.nextSibling);
        }
        
        // Rozpocznij pobieranie pogody
        fetchWeather();
    ...
    async function fetchWeather() {
        ...
        try {
            const response = await fetch(
                `https://api.openweathermap.org/data/2.5/weather?q=${CONFIG.city}&appid=${CONFIG.apiKey}&units=metric&lang=${CONFIG.lang}`
            );
            ...
            widget.querySelector('.weather-city').textContent = 'Błąd połączenia';
            widget.querySelector('.weather-city').classList.add('error');
        }
    }
    ...
    function updateWidget(data) {
        ...
        // Sprawdź próg temperatury
        if (temp < CONFIG.threshold) {
            widget.classList.add('hidden');
            console.log(`Weather Widget: Hidden - temperature ${temp}°C is below threshold ${CONFIG.threshold}°C`);
            return;
        }
        
        widget.classList.remove('hidden');
        
        // Oblicz temperaturę nocną (przybliżenie: temp_min lub temp - 5°C)
        const nightTemp = Math.round(Math.min(data.main.temp_min, temp - 5));
        
        // Aktualizuj elementy
        widget.querySelector('.weather-city').textContent = data.name;
        widget.querySelector('.weather-city').classList.remove('error');
        widget.querySelector('.weather-temp').textContent = `${temp}°C`;
        widget.querySelector('.weather-night-temp').textContent = `${nightTemp}°C`;
        widget.querySelector('.weather-footer-temp').textContent = `${temp}°C`;
        
        // Aktualizuj datę
        const now = new Date();
        const months = monthNames[CONFIG.lang] || monthNames['en'];
        const dateText = `${now.getDate()} ${months[now.getMonth()]} ${now.getFullYear()}`;
        widget.querySelector('.weather-date').textContent = dateText;
        
        // Aktualizuj ikonę
        updateWeatherIcon(widget, data.weather[0].icon);
    }
    
    // Aktualizacja ikony pogody
    function updateWeatherIcon(widget, iconCode) {
        const wrapper = widget.querySelector('.weather-icon-wrapper');
        let iconHtml = '';
